...
test("parseAckEmoji: normalizes colons/case, splits on commas and spaces, drops junk, dedupes", () => {
  assert.deepEqual(parseAckEmoji(":mirai_thinking:, MIRAI_OK  mirai_ok\n:+1:"), ["mirai_thinking", "mirai_ok", "+1"]);
...
test("config: SLACK_ACK_EMOJI lands as ackEmoji; absent or empty leaves it unset", () => {
  ...
  const withSet = slackPluginConfigFromEnv({ ...base, SLACK_ACK_EMOJI: ":mirai_thinking:,mirai_ok" });
  assert.deepEqual(withSet?.ackEmoji, ["mirai_thinking", "mirai_ok"]);
...
function pickerFixture(opts?: { candidatesOverride?: readonly string[] }) {
  ...
  const client = { emoji: { list: async () => ({ emoji: { mirai_ok: "https://x/a.png" } }) } };
...
test("ackPickCandidates: an org override fully replaces the stock candidate set", async () => {
  const { picker, client } = pickerFixture({ candidatesOverride: ["mirai_thinking", "mirai_ok"] });
  picker.refreshAckEmoji(client);
  await Promise.resolve();
  assert.deepEqual(picker.ackPickCandidates(client), ["mirai_thinking", "mirai_ok"]);
});
...
test("ack presenter: fallback reaction draws from the org set when provided", async () => {
  ...
  const presenter = createAckPresenter({
    ...
    emojiCandidates: ["mirai_thinking"],
    reactionDelayMs: 1,
  });
  await new Promise((r) => setTimeout(r, 20));
  await presenter.drain();
  assert.deepEqual(added, ["mirai_thinking"]);
